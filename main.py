    ...
    try:
        with fs.open(inputFilePath,'r') as csvinput:
            with fs.open(outputFilePath, 'w') as csvoutput:
                ...
                logging.info("Creation of csv file success - " + outputFilePath)
                #Number of rows would be length - 1 as 1st row is header
                logging.info("Number of rows - " + str(len(all)-1))
    except:
      #If there are any errors, we would return None
        logging.error("Error occured during csv read/write")
        return None
...
def load_csv_to_bigQuery(inputCsvFile, bqTableId):
    ...
    try:
        ...
        if loadJob.errors is None:
            logging.info("Big Query Load Job Successful")
        else:
            logging.error("Big Query Load Job Failed")   
            logging.error(loadJob.errors) 
            return 0
        return loadJob.output_rows
    except:
        #In case of any error/exceptions in the BQ load job we would return 0 as the row count
        logging.error("Error occured while loading data to bigquery")
        return 0    
...
def file_finalized(event, context):   
    ...
    if outputFilePath is not None:
        ...
        logging.info("Wrote {} rows to destination".format(numberOfRows))